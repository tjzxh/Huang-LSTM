...
equi_state = []
step, speed_list = 100, [20]  # list(range(5, 21))
gap = [3.438, 5.445, 7.689, 9.449, 12.122, 14.754, 17.048, 20.782, 25.678, 31.396, 39.082, 47.998, 58.115, 69.94,
       87.985, 108.745]  # , 105, 110, 115, 120, 125, 130, 135, 140, 145, 150]
...
for ind, equi_speed in enumerate(speed_list):
    ...
    for possible_gap in range(90 * step, 120 * step):  # (int(gap[ind] * step), int((gap[ind] + 1) * step)):
    ...
    local_stb = np.sum(f_s, axis=1) / (np.sum(f_v + f_vd, axis=1) - 1)
    # String stability
    dsize = f_v.shape[0]
    ...
    final_local_stb, final_string_stb, final_necessary_stb = local_stb[equi_ind], string_stb[
        equi_ind], necessary_string_stb[equi_ind]
    equi_state.append([equi_speed, equi_gap, final_local_stb, final_string_stb[0], final_necessary_stb[0]])

# Plot the equilibrium states
equi_state = np.array(equi_state)
print(equi_state[:, 0], equi_state[:, 1], equi_state[:, 1] / equi_state[:, 0], equi_state[:, -2])

# m = np.polyfit(equi_state[:, 1], equi_state[:, 0], 1)
# print(m)
plt.figure(1)
valid_string_stb = equi_state[:, 3]
...
plt.figure(2)
valid_local_stb = equi_state[:, 2]
color_local_stb = np.empty_like(valid_local_stb, dtype=str)
color_local_stb[np.where(valid_local_stb > 0)] = 'r'
color_local_stb[np.where(valid_local_stb < 0)] = 'b'
