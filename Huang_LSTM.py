...
all_veh_id.sort()
train_veh_id = sample(all_veh_id, 650)
all_input, all_output = [], []
print("Experiment starts")
for i in range(int(origin_data.shape[0]) - 50):
    if origin_data[i, 0] in train_veh_id and origin_data[i, 0] == origin_data[i + 49, 0] and origin_data[i + 50, 1] - \
            origin_data[i, 1] == 50:
        all_input.append(np.hstack((vs_scaled[i:i + 50, :], vd_scaled[i:i + 50, :])))
        all_output.append(vs_scaled[i + 50, 0])
